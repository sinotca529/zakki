...
`public`, `private`, `draft` ディレクトリ直下になく、名前も `index.md` ではないファイルは、サブページとして扱われます。
サブページは検索の対象には含まれます。
...
`create` と `update` は `YYYY-MM-DD` の形で書きます。存在しない日付を書くとビルドが止まります。
...
サイトの生成時に KaTeX が HTML へ変換するため、閲覧時に javascript は要りません。
