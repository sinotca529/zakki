...
`js_list` と `css_list` のパスは、サイトのルート (`build/`) からの位置として読みます。
先頭の `/` は付けても構いません。サイトをドメインの直下ではなくサブディレクトリに置く場合 (GitHub Pages のプロジェクトページなど) も、この書き方で通ります。<br>
外部のファイルは `https://` から始まる URL で指定します。
...
`zakki-children` のコードブロックを置くと、そこが同じディレクトリにある記事の一覧に変わります。<br>
サブページへのリンクを手で並べる必要はありません。
...
一覧に並ぶのは直下の記事だけです。ファイル名の辞書順に並びます。<br>
入れ子のディレクトリは、その `index.md` が 1 件として並びます。さらに下の記事は、そちらの一覧に出ます。<br>
子が 1 つもないときは、コードブロックごと取り除かれます。

子を持つディレクトリの `index.md` は、ページ一覧のカードに `group` クラスが付きます。<br>
`asset/style.css` の `.card.group` で見た目を変えられます。
...
update: 2024-08-15   # 記事の最終更新日 (省略すると作成日と同じになります)
