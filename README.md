...
分割規則は `src/util/tokenizer.rs` と `asset/script.js` の両方に実装されています。
片方だけを変更すると検索がヒットしなくなるため、必ず揃えてください。
