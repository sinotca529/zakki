...
インデックスを作る際、テキストは次の手順でトークンに分割されます。

まず、テキストを次の切れ目で断片に分けます。

- 空白・句読点・記号など、語を構成しない文字。これらは断片に含めず捨てます
- ASCII 英数字と、それ以外の文字 (日本語など) の切り替わり

次に、断片ごとにトークンを作ります。

- ASCII 英数字の断片は、そのまま 1 つのトークンにします (`rust` → `rust`)
- それ以外の断片は、隣り合う 2 文字の組をトークンにします (`検索語` → `検索`, `索語`)
- ただし 1 文字しかない断片は、2 文字の組を作れないので、その 1 文字をトークンにします

トークンは小文字に揃えるため、大文字と小文字は区別されません。

```txt
Rust製の検索  →  rust, 製の, の検, 検索
Rust製        →  rust, 製
あい。うえ    →  あい, うえ
```
...
なお、1 文字だけのクエリはほとんどヒットしません。
インデックスに入っているのは `検索`, `索語` のような 2 文字のトークンで、
クエリ `検` が作る 1 文字のトークンはそこに含まれないためです。
