...
index   = "(任意) 1 ファイルに複数のフォントが入っている場合 (.ttc) に、何番目を使うかを指定します。デフォルトは 0 です。"
...
同梱の Source Code Pro は日本語を持たないので、日本語は必ず別のフォントに落ちます。

`zakki.toml` の `code_font` に日本語の等幅フォントを指定すると、記事で実際に使われた文字だけを抜き出したフォントを生成し、サイトと一緒に配ります。
